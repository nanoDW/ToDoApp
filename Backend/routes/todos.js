...
router.post("/", auth, async (req, res) => {
  const { error } = validate.validateTodo(req.body);
...
router.get("/me", auth, async (req, res) => {
  const user = await Schemas.User.findById(req.user._id).select("nick");
  const todos = await Schemas.Todo.find({
    nick: user.nick
  });
  if (!todos[0]) return res.status(404).send("You have no todos yet");
  res.send(todos);
});
...
router.put("/todos/:id", auth, async (req, res) => {
  const { error } = validate.validateTodoEdited(req.body);
  if (error) return res.status(400).send(error.details[0].message);
